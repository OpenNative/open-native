...
public @interface ReactMethod {
   ...
   * the hook can only be void or return JSON values (e.g. bool, number, String, {
   * WritableMap}, or {WritableArray}). Calling these methods when running under the websocket
   * executor is currently not supported.
   */
  boolean isBlockingSynchronousMethod() default false;
}

// We need this here so react native modules don't start crying again.
// In NativeScript we don't need this actually at all since we can
// and we will access java classes directly.
